...
from .consts import TW_DEFAULT_PROFILE_IMG, UNKNOWN_LANG
...
class M3Twitter(M3Inference):
    def __init__(
        self,
        cache_dir=expanduser("~/m3/cache"),
        model_dir=expanduser("~/m3/models/"),
        pretrained=True,
        use_full_model=True,
        use_cuda=True,
        parallel=False,
    ...
    def transform_jsonl(
        ...
        with open(input_file, "r") as fhIn:
            with open(output_file, "w") as fhOut:
                for line in fhIn:
                    m3vals = self.transform_jsonl_object(
                        line,
    ...
    def transform_jsonl_object(
        ...
        if img_path_key != None and img_path_key in user:
            ...
            if resize_img:
                img_file_resize = "{}/{}_224x224.{}".format(
                    self.cache_dir, user["id_str"], get_extension(img_path)
                )
            ...
            dotpos = img_path.rfind(".")
            img_file_full = "{}/{}.{}".format(
                self.cache_dir, user["id_str"], img_path[dotpos + 1 :]
            )
            img_file_resize = "{}/{}_224x224.{}".format(
                self.cache_dir, user["id_str"], get_extension(img_path)
            )
    ...
    def _twitter_api(self, id=None, screen_name=None):
        ...
        if screen_name != None:
            ...
            try:
                r = self.twitter_session.get(
                    "users/show.json", params={"screen_name": screen_name}
                )
            except:
    ...
    def infer_id(self, id, skip_cache=False):
        ...
        if not skip_cache:
            ...
            logger.info(
                "skip_cache is True. Fetching data from Twitter for id {}.".format(id)
            )
    ...
    def process_twitter(self, data):
        ...
        if img_path == "" or "default_profile" in img_path:
            ...
            img_file_full = f"{self.cache_dir}/{id}" + (
                f".{img_path[img_path.rfind('.') + 1:]}"
                if "." in img_path.split("/")[-1]
                else ""
            )
            img_file_resize = "{}/{}_224x224.{}".format(
                self.cache_dir, id, get_extension(img_path)
            )
        ...
        output = {"input": data[0], "output": pred[id]}
        return output
